...
task :create_inventory_in_out_daily_view => :environment do
  ...
  CSV.open(file, 'w', write_headers: true, headers:headers) do |writer|
    ...
    organizations.each do |org|
      ...
      storage_locations = org.storage_locations.active_locations.order(:id)
      storage_locations.each do |loc|
        puts "#{org.id}, #{org.name}, #{loc.name}"
        lii = ItemsInQuery.new(organization: org, storage_location: loc).call
        line_items_out = ItemsOutQuery.new(organization: org, storage_location: loc).call
        inventory = loc.inventory_items
        line_items_in = lii.sort_by {|li| li.item_id}
